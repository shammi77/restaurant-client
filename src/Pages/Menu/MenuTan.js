import React from 'react';

const MenuTan = () => {
    return (
        <div>
        <div className="bg-gradient-to-r from-red-500 to-red-800 m-20">
   <h2 className=" text-3xl text-center font-bold mt-10 mb-20">
   ミ★ 𝘚𝘌𝘎𝘜𝘕𝘋𝘖𝘚 𝘗𝘓𝘈𝘛𝘖𝘚 𝘛𝘈𝘕𝘋𝘖𝘖𝘙𝘐 (𝘚𝘐𝘕 𝘚𝘈𝘓𝘚𝘈) / <br/>𝘛𝘈𝘕𝘋𝘖𝘖𝘙𝘐 𝘔𝘈𝘐𝘕 𝘔𝘌𝘈𝘓 𝘋𝘐𝘚𝘏𝘌𝘚 ★彡
   </h2>
   <h1 className="ml-56  italic text-amber-400">
     Todos los platos estan hechos al tandoori (horno indio de barro) y servido con ensalada.
   </h1>
   <p className="ml-56 italic text-white">All dishes are served with salad.</p>
   <br />
   
   <h1 className="ml-56  font-bold text-white text-xl">
     MASALA GRIL ............... 17.50 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
     Incluye: pollo tandoori, pollo tikka, sheek kebab, langotinos tandoori.
   </h1>
   <p className="ml-56 italic text-white">
    Include: Chicken tandoori,chicken tikka, lamb tikka, sheek kebab, tandoori king prawns.
   </p>
   <br />
   <h1 className="ml-56  font-bold text-white text-xl">
     POLLO TANDOORI / CHICKEN TANDOORI .................................................................
     13.50 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
     Muslo y alas de pollo marinado hechas al tandoori.
   </h1>
   <p className="ml-56 italic text-white">
     Chicken leg and breast grill in tandoori.
   </p>
   <br />
   <h1 className="ml-56  font-bold text-white text-xl">
     POLLO TIKKA / CHICKEN TIKKA
     ..................................................................
     12.75 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
     Pinchitos de pollo sin hueso.
   </h1>
   <p className="ml-56 italic text-white">
   Pieces of chicken made on tandoori.
   </p>
   <br />
   <h1 className="ml-56  font-bold text-white text-xl">
     POLLO SHASLIK / CHICKEN SHASLIK
     ..................................................................
     12.90 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
     Pollo tikka, pimento verde, rojo y cebollas hechas al tandoori.
   </h1>
   <p className="ml-56 italic text-white">
     Pieces of chicken, red pepper, green pepper made on tandoori.
   </p>
   <br/>


   <h1 className="ml-56  font-bold text-white text-xl">
     CORDERO TIKKA / LAMB TIKKA
     ..................................................................
     14.50 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
   Trozos de carne de cordero hechos al tandoori.
   </h1>
   <p className="ml-56 italic text-white">
   Pieces of lamb made on tandoori.
   </p>
   <br/>

   <h1 className="ml-56  font-bold text-white text-xl">
    SHEEK KEBAB 
     ..................................................................
     14.50 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
   Carne picada hechos en tandoori.
   </h1>
   <p className="ml-56 italic text-white">
   Minced meat made on tandoori.
   </p>
   <br/>


   <h1 className="ml-56  font-bold text-white text-xl">
     CORDERO SHASLIK / LAMB SHASLIK
     ..................................................................
     14.75 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
   Coredero tikka, pimento verdo, rojo y cebollas hechas al tandoori.
   </h1>
   <p className="ml-56 italic text-white">
   Pieces of lamb, red peppers, green peppers and onion made on tandoori.
   </p>
   <br/>
   <h1 className="ml-56  font-bold text-white text-xl">
     LANGOSTINO TANDOORI / KING PRAWNS
     ..................................................................
     15.50 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
   langotinos pelados hechos al tandoori.
   </h1>
   <p className="ml-56 italic text-white">
   King prawns made on tandoori.
   </p>
   <br/>
   <h1 className="ml-56  font-bold text-white text-xl">
     LUBINA AL TANDOORI / TANDOORI SEA BASS
     ..................................................................
     16.50 €
   </h1>
   <h1 className="ml-56  italic text-amber-400">
   Lubina hechas al tandoori.
   </h1>
   <p className="ml-56 italic text-white">
   Sea bass made on tandoori.
   </p>
   <br/>

    </div>
   </div>
    );
};

export default MenuTan;